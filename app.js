...
onAuthStateChanged(auth, user => {
  ...
  if (user) { showApp(); startLiveSync(user.uid); } else { showLogin(); }
});

/* === Kategorien-Konfiguration laden/speichern === */
...
function initDatePickers() {
  ...
  document.querySelectorAll('input[type="date"]').forEach(input => {
    ...
    flatpickr(input, { dateFormat: 'Y-m-d', altInput: true, altFormat: 'd.m.Y', disableMobile: true });
  });
}

function bindEnterToSave(modalEl, saveFn) {
  if (modalEl._enterHandler) modalEl.removeEventListener('keydown', modalEl._enterHandler);
  const handler = e => {
...
function showLogin() {
  ...
  state.tasks = []; state.todos = []; state.hours = [];
  renderStats(); renderTasks(); renderTodos(); renderGantt(); renderHours();
}
...
function startLiveSync(uid) {
  const tasksUnsub = onSnapshot(collection(db, 'users', uid, 'tasks'), snap => {
    state.tasks = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
    renderStats(); renderTasks(); renderGantt();
  });
  const todosUnsub = onSnapshot(collection(db, 'users', uid, 'todos'), snap => {
    state.todos = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
    renderTodos();
  });
  const hoursUnsub = onSnapshot(collection(db, 'users', uid, 'hours'), snap => {
    state.hours = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (b.date || '').localeCompare(a.date || ''));
    renderHours();
  });
...
function taskDocRef(id) { return doc(db, 'users', state.user.uid, 'tasks', id); }
function todoDocRef(id) { return doc(db, 'users', state.user.uid, 'todos', id); }
function hoursDocRef(id) { return doc(db, 'users', state.user.uid, 'hours', id); }
...
function renderStats() {
  ...
  bar.innerHTML =
    '<div class="stat-card stat-main"><span class="stat-label">Offene Hauptaufgaben</span><span class="stat-value">' + openMain + '</span></div>' +
    '<div class="stat-card stat-sub"><span class="stat-label">Offene Unteraufgaben</span><span class="stat-value">' + openSub + '</span></div>' +
    '<div class="stat-card stat-budget"><span class="stat-label">Gesamtbudget</span><span class="stat-value">' + totalBudget.toLocaleString('de-DE') + ' €</span></div>';
}
...
function populateOwnerFilter() {
  ...
  sel.innerHTML = '<option value="all">Alle Verantwortlichen</option>' + owners.map(o => '<option value="' + escHtml(o) + '">' + escHtml(o) + '</option>').join('');
...
function getFilteredMainTasks() {
  ...
  const filtered = state.tasks.filter(t => !t.parentId).filter(t => {
    ...
    const matchesQuery = !q || haystacks.some(x => (x.title || '').toLowerCase().includes(q) || (x.note || '').toLowerCase().includes(q) || (x.owner || '').toLowerCase().includes(q));
    const matchesStatus = statusFilter === 'all' || haystacks.some(x => x.status === statusFilter);
    const matchesOwner = ownerFilter === 'all' || haystacks.some(x => x.owner === ownerFilter);
...
function renderTasks() {
  ...
  const filterActive = document.getElementById('search-input').value.trim() !== '' || document.getElementById('filter-status').value !== 'all' || document.getElementById('filter-owner').value !== 'all';
  if (!tasks.length) {
    container.innerHTML = '<div class="empty-state"><div class="icon">📋</div>Keine Aufgaben gefunden.</div>';
    return;
  }
...
function taskCardHTML(task) {
  ...
  const dateTag = (task.startDate || task.endDate) ? '<span class="tag">📅 ' + (task.startDate||'?') + ' → ' + (task.endDate||'?') + '</span>' : '';
  const budgetTag = task.budget ? '<span class="tag budget">💶 ' + Number(task.budget).toLocaleString('de-DE') + ' €</span>' : '';
  const ownerTag = task.owner ? '<span class="tag task-owner">👤 ' + escHtml(task.owner) + '</span>' : '';
...
function bindDragEvents() {
  ...
  container.querySelectorAll('.task-card').forEach(card => {
    ...
    card.addEventListener('dragend', () => { card.classList.remove('dragging'); container.querySelectorAll('.task-card').forEach(c => c.classList.remove('drag-over')); });
    card.addEventListener('dragover', e => { e.preventDefault(); if (card === dragSrc) return; container.querySelectorAll('.task-card').forEach(c => c.classList.remove('drag-over')); card.classList.add('drag-over'); });
    card.addEventListener('drop', e => { e.preventDefault(); if (!dragSrc || card === dragSrc) return; const rect = card.getBoundingClientRect(); const before = e.clientY < rect.top + rect.height / 2; container.insertBefore(dragSrc, before ? card : card.nextSibling); card.classList.remove('drag-over'); persistNewOrder(); });
  });
}
...
function openTaskModal(id, parentId) {
  ...
  const isNew = !id;
  document.getElementById('modal-task-title').textContent = isNew ? (parentId ? 'Neue Unteraufgabe' : 'Neue Aufgabe') : 'Aufgabe bearbeiten';
  ...
  if (!isNew) {
    ...
    ['ti-id','ti-title','ti-start','ti-end','ti-budget','ti-owner','ti-note','ti-link','ti-file'].forEach(i => { document.getElementById(i).value = ''; });
...
async function saveTask() {
  ...
  if (fileInput && fileInput.files[0]) {
    try { attachmentUrl = await uploadTaskFile(fileInput.files[0], id); }
    catch (err) { alert('❌ Datei-Upload fehlgeschlagen: ' + (err && err.message ? err.message : 'Unbekannter Fehler')); }
  }
  const task = {
    id,
...
async function deleteTask(id) {
  ...
  const ids = state.tasks.filter(t => t.id === id || t.parentId === id).map(t => t.id);
...
function renderGantt() {
  ...
  const mainTasks = state.tasks.filter(t => !t.parentId).sort((a, b) => (a.order ?? a.createdAt ?? 0) - (b.order ?? b.createdAt ?? 0));
  const ordered = [];
  mainTasks.forEach(main => {
    ...
    subtasks.forEach(st => { if (st.startDate || st.endDate) ordered.push({ task: st, isMain: false }); });
  });
  if (!ordered.length) { container.innerHTML = '<div class="empty-state"><div class="icon">📅</div>Noch keine Aufgaben mit Daten vorhanden.</div>'; return; }
  const allDates = ordered.flatMap(o => [o.task.startDate, o.task.endDate].filter(Boolean)).map(d => new Date(d));
  ...
  const rows = ordered.map(o => {
    ...
    const todayLine = todayInRange ? '<div class="gantt-today-line" style="left:' + todayLeft + '%"></div>' : '';
  ...
  const legend = todayInRange ? '<div class="gantt-today-legend"><span class="dot"></span>Heute (' + fmtDate(today) + ')</div>' : '';
  container.innerHTML = legend + '<div class="gantt-wrapper"><table class="gantt-table"><thead><tr>' +
    '<th class="gantt-col-title">Aufgabe</th><th class="gantt-col-status">Status</th><th class="gantt-col-date">Start</th><th class="gantt-col-date">Ende</th>' +
...
function renderTodos() {
  ...
  if (!todos.length) { container.innerHTML = '<div class="empty-state"><div class="icon">✅</div>Keine To-Dos vorhanden.</div>'; return; }
...
async function toggleTodo(id) {
  ...
  const update = { done: nowDone, completedAt: nowDone ? new Date().toISOString().split('T')[0] : null };
...
function populateHoursWorkerFilter() {
  ...
  sel.innerHTML = '<option value="all">Alle Helfer</option>' + workers.map(w => '<option value="' + escHtml(w) + '">' + escHtml(w) + '</option>').join('');
...
function populateHoursTaskSelect() {
  ...
  const mainTasks = state.tasks.filter(t => !t.parentId);
  sel.innerHTML = '<option value="">– Keine –</option>' + mainTasks.map(t => '<option value="' + t.id + '">' + escHtml(t.title) + '</option>').join('');
}
...
function renderHours() {
  ...
  bar.innerHTML =
    '<div class="stat-card stat-main"><span class="stat-label">Gesamtstunden</span><span class="stat-value">' + totalHours.toLocaleString('de-DE', { minimumFractionDigits: 1 }) + ' Std</span></div>' +
    '<div class="stat-card stat-sub"><span class="stat-label">Helfer</span><span class="stat-value">' + uniqueWorkers + '</span></div>';
  if (!entries.length) { container.innerHTML = '<div class="empty-state"><div class="icon">🕒</div>Noch keine Stunden erfasst.</div>'; return; }
  const grouped = {};
  entries.forEach(h => { const key = h.worker || 'Unbekannt'; if (!grouped[key]) grouped[key] = []; grouped[key].push(h); });
  ...
  container.innerHTML = workerNames.map(worker => {
    ...
    const itemsHTML = items.map(h => {
      ...
      return '<div class="hours-card" data-id="' + h.id + '">' +
        '<div class="hours-info"><div class="hours-meta">📅 ' + (h.date || '—') + (task ? ' · 🔧 ' + escHtml(task.title) : '') + '</div></div>' +
...
async function saveHoursEntry() {
  ...
  if (!worker || !date || !totalHours || totalHours <= 0) { alert('Bitte Helfer, Datum und Gesamtstunden ausfüllen.'); return; }
...
(function initPullToRefresh() {
  ...
  function isAnyModalOpen() { return [modalTask, modalTodo, modalHours, modalPin].some(m => !m.classList.contains('hidden')); }
  function getScrollTop() { const activePage = document.querySelector('.page.active'); return activePage ? activePage.scrollTop || window.scrollY : window.scrollY; }
